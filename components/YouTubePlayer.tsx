...
export default function YouTubePlayer({ id, title, thumbnail }: { id: string; title: string; thumbnail?: string }) {
  ...
  if (playing) return <iframe className="aspect-video w-full border-0" src={`https://www.youtube-nocookie.com/embed/${id}?autoplay=1`} title={title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen referrerPolicy="strict-origin-when-cross-origin" />;
  return (
